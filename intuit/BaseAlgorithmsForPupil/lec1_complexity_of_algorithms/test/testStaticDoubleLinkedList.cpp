...
#ifdef PRACTICE
#include <practice/StaticDoubleLinkedList.hpp>
using namespace lec1::practice;
#else
#include <include/StaticDoubleLinkedList.hpp>
...
TEST(StaticDoubleLinkedList, sameListIfAddedFrontBack) {
  StaticDoubleLinkedList<10> list0, list1;
...
TEST(StaticDoubleLinkedList, addBackTillFull) {
  constexpr int MAX_N = 10;
  StaticDoubleLinkedList<MAX_N> list;
...
TEST(StaticDoubleLinkedList, addFrontTillFull) {
  constexpr int MAX_N = 10;
  StaticDoubleLinkedList<MAX_N> list;
...
TEST(StaticDoubleLinkedList, empty) {
  constexpr int MAX_N = 10;
  StaticDoubleLinkedList<MAX_N> list;
  ...
  list.add_front(1);
  EXPECT_TRUE(!list.empty());
  list.remove(list.next[list.head]);
  EXPECT_TRUE(list.empty());
...
TEST(StaticDoubleLinkedList, remove) {
...
TEST(StaticDoubleLinkedList, addAfter) {
  int a[] = {1, 2, 3, 4, 5};
  constexpr int MAX_N = size(a) * 2;
  StaticDoubleLinkedList<MAX_N> list;
  list.add_back(1);
...
TEST(StaticDoubleLinkedList, addFrontAddBackInterleaved) {
  int a[] = {1,2,3,4,5,6,7,8,9};
  StaticDoubleLinkedList<size(a)> list;
